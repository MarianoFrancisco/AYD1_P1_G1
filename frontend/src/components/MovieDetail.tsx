...
export const MovieDetail: React.FC<MovieDetailProps> = ({ id_user, alquilado }) => {
  ...
  useEffect(() => {
    const fetchMovieData = async () => {
      try {
        ...
        if (alquilado) {
          const rentedResponse = await fetch(`${import.meta.env.VITE_API_URL}/api/alquileres/${id_user}/1`)
  ...
  return (
    <div className="flex justify-center bg-gray-50" style={{ margin: '10px' }}>
      {movieData.map(movie => (
        <div key={movie.id_pelicula} className="max-w-5xl w-full p-4 rounded-lg shadow-md relative">
          <div className="flex mb-2">
            ...
            <div className="flex-1 p-4 bg-white rounded-lg shadow-lg">
              ...
              <div className="flex">
                {!alquilado ? (
                  <button className="mt-3 mr-2 px-4 py-2 text-sm bg-gray-800 text-white rounded-md hover:bg-gray-700 transition-all" onClick={handleAlquilar}>
                    Alquilar
                  </button>
                ) : (
                  <button className="mt-3 mr-2 px-4 py-2 text-sm bg-gray-800 text-white rounded-md hover:bg-gray-700 transition-all" onClick={handleDevolver}>
                    Devolver
                  </button>
                )}
          ...
          {showComments && (
            <div className="fixed top-0 right-0 w-full h-full flex items-center justify-center bg-black bg-opacity-50" style={{ zIndex: 9999 }}>
              <div className="bg-white max-w-6xl p-4 rounded-lg shadow-lg" style={{ height: '70vh', overflowY: 'auto', width: '50vw', position: 'relative' }}>
                ...
                <CommentsSection id_user={id_user} id_pelicula={movie.id_pelicula} />
              </div>
            </div>
          )}
        </div>
